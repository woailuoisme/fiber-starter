...
func RegisterSampleTasks() {
	// 1. 每隔五分钟清理一次临时文件
	schedule.Job(monitoringJobs.NewCleanupTempFilesJob("/tmp/exports")).EveryFiveMinutes()

	// 2. 每天凌晨 3:00 发送汇总提醒
	schedule.Job(userJobs.NewReminderNotificationJob(1, "Daily Report Ready")).DailyAt("03:00")

	// 3. 每周一上午 9:00 执行
	schedule.Job(userJobs.NewReminderNotificationJob(1, "Weekly Planning")).WeeklyOn(1, "09:00")
}
