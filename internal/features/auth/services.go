...
const (
	authOTPPurposeSignup        = AuthOTPPurposeSignup
	authOTPPurposePasswordReset = AuthOTPPurposePasswordReset
...
func (s *authService) userRepo() (*userPkg.UserRepository, error) {
	db, err := s.bunDB()
	if err != nil {
		return nil, err
	}
	return userPkg.NewUserRepository(db), nil
}

func (s *authService) authRepo() (*AuthRepository, error) {
	db, err := s.bunDB()
	if err != nil {
		return nil, err
	}
...
func (s *authService) Register(ctx context.Context, input RegisterInput) (SignUpResult, error) {
	...
	err = bunDB.RunInTx(ctx, nil, func(ctx context.Context, tx bun.Tx) error {
		userRepo := userPkg.NewUserRepository(tx)
...
func (s *authService) issueSessionTokens(user *userPkg.User) (string, string, error) {
...
func (s *authService) issueOTP(email string, purpose AuthOTPPurpose) (string, *AuthOTP, bool, error) {
	now := helpers.UtcNow()
...
func (s *authService) createOTP(email string, purpose AuthOTPPurpose, code string, now time.Time) (*AuthOTP, error) {
...
func (s *authService) getLatestOTP(email string, purpose AuthOTPPurpose) (*AuthOTP, error) {
	ctx := context.Background()
	authRepo, err := s.authRepo()
	if err != nil {
		return nil, err
...
func (s *authService) validateOTP(authRepo *AuthRepository, email string, purpose AuthOTPPurpose, code string) (*AuthOTP, error) {
	ctx := context.Background()
...
func hashOTPCode(secret, email string, purpose AuthOTPPurpose, code string) string {
