			...
			for _, path := range envPaths {
				if fileExists(path) {
					...
					if err := v.ReadInConfig(); err == nil {
						...
						log.Printf("Successfully loaded environment file: %s", path)
						loaded = true
						break
					}
				}
...
// LoadDatabaseConfig Load database configuration file
func LoadDatabaseConfig() (*DatabaseConfig, error) {
	...
	// Set database configuration file path and name
	...
	// Manually handle environment variable replacement
	replaceEnvVars()
	...
	// Check if DB_CONNECTION environment variable exists, if so override default
...
func LoadConfig() (*Config, error) {
	// First load database configuration
	dbConfig, err := LoadDatabaseConfig()
	if err != nil {
		log.Printf("Failed to load database configuration: %v", err)
		// If database configuration loading fails, use default configuration
		dbConfig = &DatabaseConfig{}
	}
	...
	// Manually override environment variables
	overrideWithEnv()
