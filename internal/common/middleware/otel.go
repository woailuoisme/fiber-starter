...
func SetupOTEL(app *fiber.App, cfg *configs.Config) {
	if cfg == nil || (!cfg.OTEL.TraceEnabled && !cfg.OTEL.MetricsEnabled) {
		return
	}

	app.Use(fiberotel.Middleware(
		fiberotel.WithoutMetrics(!cfg.OTEL.MetricsEnabled),
		fiberotel.WithSpanNameFormatter(func(c fiber.Ctx) string {
			return c.Method() + " " + c.Route().Path
		}),
		fiberotel.WithCustomAttributes(func(c fiber.Ctx) []attribute.KeyValue {
			if reqID := getRequestID(c); reqID != "" {
				return []attribute.KeyValue{attribute.String("request_id", reqID)}
			}
			return nil
		}),
