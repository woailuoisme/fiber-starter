...
//	@Success		201		{object}	helpers.APIResponse{data=object{user=models.SafeUser,verification_required=bool}}	"注册成功"
...
func (c *AuthController) SignUp(ctx fiber.Ctx) error {
	...
	return helpers.HandleCreated(ctx, "Registered successfully", resources.NewSignUpResource(result).ToResponse())
}
...
//	@Accept			json
//	@Produce		json
//	@Param			request	body		requests.VerifySignUpRequest																					true	"验证参数"
//	@Success		200		{object}	helpers.APIResponse{data=object{user=models.SafeUser,tokens=object{access_token=string,refresh_token=string}}}	"验证成功"
//	@Router			/api/v1/auth/sign-up/verify [post]
func (c *AuthController) VerifySignUp(ctx fiber.Ctx) error {
	var req requests.VerifySignUpRequest
	...
	return helpers.HandleSuccess(ctx, "Email verified successfully", resources.NewAuthResultResource(result).ToResponse())
}
...
//	@Success		200		{object}	helpers.APIResponse{data=object{user=models.SafeUser,tokens=object{access_token=string,refresh_token=string}}}	"登录成功"
...
func (c *AuthController) SignIn(ctx fiber.Ctx) error {
	...
	return helpers.HandleSuccess(ctx, "Logged in successfully", resources.NewAuthResultResource(result).ToResponse())
}
...
//	@Accept			json
//	@Produce		json
//	@Param			request	body		requests.RefreshTokenRequest												true	"刷新参数"
//	@Success		200		{object}	helpers.APIResponse{data=object{access_token=string,refresh_token=string}}	"刷新成功"
...
func (c *AuthController) RefreshSession(ctx fiber.Ctx) error {
	...
	return helpers.HandleSuccess(ctx, "Token refreshed successfully", resources.NewAuthTokensResource(tokens).ToResponse())
}
...
//	@Produce		json
//	@Security		Bearer
//	@Success		200	{object}	helpers.APIResponse	"注销成功"
...
//	@Produce		json
//	@Security		Bearer
//	@Param			request	body		requests.ChangePasswordRequest	true	"修改参数"
...
func (c *AuthController) UpdatePassword(ctx fiber.Ctx) error {
	...
	if err := c.authService.ChangePassword(ctx.Context(), req.ToInput(user.ID)); err != nil {
		return helpers.HandleAppError(ctx, err)
	}
...
//	@Accept			json
//	@Produce		json
//	@Param			request	body		requests.VerifyResetPasswordRequest						true	"验证重置密码参数"
//	@Success		200		{object}	helpers.APIResponse{data=object{reset_token=string}}	"验证成功"
//	@Router			/api/v1/auth/reset-password/verify [post]
func (c *AuthController) VerifyPasswordReset(ctx fiber.Ctx) error {
	var req requests.VerifyResetPasswordRequest
