...
func (r *AuthRepository) CreateOTP(ctx context.Context, otp *models.AuthOTP) error {
	_, err := r.db.NewInsert().Model(otp).Returning("*").Exec(ctx)
	return err
}

func (r *AuthRepository) GetLatestOTPByEmailPurpose(ctx context.Context, email string, purpose models.AuthOTPPurpose) (*models.AuthOTP, error) {
	var otp models.AuthOTP
	err := r.db.NewSelect().
		Model(&otp).
		Where("email = ?", email).
