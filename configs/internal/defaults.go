package internal

func DefaultConfigMap() map[string]any {
	return map[string]any{
		"app.name":                                          "lfiber",
		"app.env":                                           "development",
		"app.debug":                                         false,
		"app.port":                                          "8080",
		"app.host":                                          "0.0.0.0",
		"app.timezone":                                      "UTC",
		"app.url":                                           "http://localhost:8080",
		"app.fiber.prefork":                                 false,
		"app.fiber.server_header":                           "",
		"app.fiber.body_limit":                              4 * 1024 * 1024,
		"app.fiber.concurrency":                             256 * 1024,
		"app.fiber.read_buffer_size":                        16 * 1024,
		"app.fiber.read_timeout":                            30,
		"app.fiber.write_timeout":                           30,
		"app.fiber.idle_timeout":                            120,
		"app.fiber.trust_proxy":                             false,
		"app.fiber.proxy_header":                            "X-Forwarded-For",
		"app.fiber.stream_request_body":                     false,
		"app.fiber.immutable":                               false,
		"database.default":                                  "postgres",
		"database.connections.postgres.driver":              "postgres",
		"database.connections.postgres.host":                "localhost",
		"database.connections.postgres.port":                "5432",
		"database.connections.postgres.database":            "lfiber",
		"database.connections.postgres.username":            "postgres",
		"database.connections.postgres.password":            "",
		"database.connections.postgres.charset":             "utf8",
		"database.connections.postgres.prefix":              "",
		"database.connections.postgres.schema":              "public",
		"database.connections.postgres.sslmode":             "disable",
		"database.connections.postgres.timezone":            "UTC",
		"database.connections.pgsql.driver":                 "postgres",
		"database.connections.pgsql.host":                   "localhost",
		"database.connections.pgsql.port":                   "5432",
		"database.connections.pgsql.database":               "lfiber",
		"database.connections.pgsql.username":               "postgres",
		"database.connections.pgsql.password":               "",
		"database.connections.pgsql.charset":                "utf8",
		"database.connections.pgsql.prefix":                 "",
		"database.connections.pgsql.schema":                 "public",
		"database.connections.pgsql.sslmode":                "disable",
		"database.connections.pgsql.timezone":               "UTC",
		"database.connections.sqlite.driver":                "sqlite",
		"database.connections.sqlite.database":              "./database/database.sqlite",
		"database.connections.sqlite.prefix":                "",
		"database.pool.max_open_conns":                      100,
		"database.pool.max_idle_conns":                      10,
		"database.pool.conn_max_lifetime":                   3600,
		"database.pool.conn_max_idle_time":                  600,
		"database.retry_attempts":                           1,
		"database.retry_backoff_ms":                         100,
		"database.retry_backoff_factor":                     2.0,
		"database.retry_on_open":                            true,
		"database.retry_on_query":                           false,
		"database.log_queries":                              false,
		"database.slow_query_threshold_ms":                  0,
		"database.migrations.table":                         "migrations",
		"database.migrations.path":                          "./database/migrations",
		"database.seeders.path":                             "./database/seeders",
		"excel.temp_path":                                   ".cache/excel",
		"database.redis.client":                             "default",
		"queue.enabled":                                     true,
		"queue.concurrency":                                 10,
		"cache.enabled":                                     true,
		"cache.driver":                                      "redis",
		"cache.prefix":                                      "lfiber_cache",
		"cache.default":                                     0,
		"cache.ttl":                                         3600,
		"redis.enabled":                                     true,
		"redis.host":                                        "127.0.0.1",
		"redis.port":                                        "6379",
		"redis.password":                                    "",
		"redis.db":                                          0,
		"jwt.secret":                                        "",
		"auth.default":                                      "api",
		"auth.guards.api.driver":                            "jwt",
		"auth.guards.api.provider":                          "users",
		"auth.guards.admin.driver":                          "jwt",
		"auth.guards.admin.provider":                        "admins",
		"auth.providers.users.driver":                       "database",
		"auth.providers.users.table":                        "users",
		"auth.providers.admins.driver":                      "database",
		"auth.providers.admins.table":                       "admin_users",
		"authorization.model_file":                          "configs/casbin/model.conf",
		"authorization.policy_file":                         "configs/casbin/policy.csv",
		"jwt.expiration_time":                               86400,
		"jwt.refresh_time":                                  604800,
		"jwt.expire_hours":                                  24,
		"jwt.issuer":                                        "lfiber",
		"logger.level":                                      "info",
		"logger.format":                                     "json",
		"logger.output":                                     "stack",
		"logger.max_size":                                   100,
		"logger.max_age":                                    30,
		"logger.max_backups":                                10,
		"logger.compress":                                   true,
		"mail.enabled":                                      true,
		"mail.default":                                      "resend",
		"mail.from_name":                                    "lfiber",
		"mail.from_address":                                 "noreply@example.com",
		"mail.reply_to":                                     "",
		"mail.api_key":                                      "",
		"mail.host":                                         "localhost",
		"mail.port":                                         1025,
		"mail.username":                                     "",
		"mail.password":                                     "",
		"mail.encryption":                                   "",
		"mail.theme.primary_color":                          "#4f46e5",
		"mail.theme.success_color":                          "#10b981",
		"mail.theme.danger_color":                           "#f43f5e",
		"mail.theme.warning_color":                          "#f59e0b",
		"mail.theme.bg_color":                               "#f8fafc",
		"notification.gotify.enabled":                       false,
		"notification.gotify.url":                           "",
		"notification.gotify.token":                         "",
		"notification.gotify.title":                         "lfiber",
		"notification.gotify.priority":                      5,
		"notification.telegram.enabled":                     false,
		"notification.telegram.api_url":                     "https://api.telegram.org",
		"notification.telegram.bot_token":                   "",
		"notification.telegram.chat_id":                     "",
		"notification.telegram.parse_mode":                  "",
		"storage.enabled":                                   true,
		"storage.driver":                                    "garage",
		"storage.database":                                  "./database/storage.db",
		"storage.reset":                                     false,
		"storage.gc_interval":                               3600,
		"storage.garage.endpoint":                           "",
		"storage.garage.access_key_id":                      "",
		"storage.garage.secret_access_key":                  "",
		"storage.garage.use_ssl":                            false,
		"storage.garage.bucket":                             "",
		"storage.garage.region":                             "",
		"storage.minio.endpoint":                            "",
		"storage.minio.access_key_id":                       "",
		"storage.minio.secret_access_key":                   "",
		"storage.minio.use_ssl":                             false,
		"storage.minio.bucket":                              "",
		"storage.minio.region":                              "",
		"storage.s3.access_key_id":                          "",
		"storage.s3.secret_access_key":                      "",
		"storage.s3.region":                                 "",
		"storage.s3.bucket":                                 "",
		"storage.s3.endpoint":                               "",
		"storage.r2.access_key_id":                          "",
		"storage.r2.secret_access_key":                      "",
		"storage.r2.region":                                 "",
		"storage.r2.bucket":                                 "",
		"storage.r2.endpoint":                               "",
		"storage.oss.access_key_id":                         "",
		"storage.oss.secret_access_key":                     "",
		"storage.oss.region":                                "",
		"storage.oss.bucket":                                "",
		"storage.oss.endpoint":                              "",
		"storage.local.root":                                "./storage/app",
		"storage.local.url":                                 "/storage",
		"storage.public.root":                               "./storage/app/public",
		"storage.public.url":                                "/storage",
		"media_library.conversion_mode":                     "sync",
		"media_library.queue":                               "default",
		"backup.disk":                                       "local",
		"backup.path":                                       "backups",
		"backup.temp_path":                                  ".cache/backup",
		"backup.notifications.enabled":                      true,
		"backup.notifications.notify_success":               false,
		"backup.notifications.channels":                     []string{"mail"},
		"backup.notifications.mail_to":                      "",
		"backup.binaries.pg_dump":                           "pg_dump",
		"backup.binaries.psql":                              "psql",
		"backup.binaries.sqlite3":                           "sqlite3",
		"websocket.enabled":                                 true,
		"websocket.port":                                    "3001",
		"websocket.path":                                    "/app/:appKey",
		"websocket.sse_path":                                "/sse/app/:appKey",
		"websocket.auth_path":                               "/broadcasting/auth",
		"websocket.app_id":                                  "lfiber",
		"websocket.app_key":                                 "lfiber",
		"websocket.app_secret":                              "",
		"websocket.bus_mode":                                "redis",
		"websocket.redis_prefix":                            "realtime",
		"websocket.write_queue_size":                        128,
		"websocket.max_message_size":                        65536,
		"websocket.heartbeat_interval":                      30,
		"websocket.presence_ttl":                            120,
		"websocket.url":                                     "http://localhost:8000/api",
		"websocket.client_url":                              "ws://localhost:8000/connection/websocket",
		"websocket.client_sse_url":                          "http://localhost:8000/connection/sse",
		"websocket.api_key":                                 "lfiber-centrifugo-api-key",
		"websocket.secret":                                  "lfiber-centrifugo-secret",
		"payment.wechat.app_id":                             "",
		"payment.wechat.mch_id":                             "",
		"payment.wechat.api_key":                            "",
		"payment.wechat.cert_path":                          "",
		"payment.wechat.key_path":                           "",
		"payment.wechat.notify_url":                         "",
		"payment.alipay.app_id":                             "",
		"payment.alipay.private_key":                        "",
		"payment.alipay.public_key":                         "",
		"payment.alipay.notify_url":                         "",
		"business.order.payment_timeout":                    30,
		"business.order.pickup_timeout":                     1440,
		"business.device.channel_count":                     53,
		"business.device.channel_max_capacity":              4,
		"security.cors.allowed_origins":                     "http://localhost:3000",
		"security.cors.allowed_methods":                     "GET,POST,HEAD,PUT,DELETE,PATCH,OPTIONS",
		"security.cors.allowed_headers":                     "Origin,Content-Type,Accept,Authorization,Cache-Control,X-Requested-With",
		"security.rate_limit.max":                           100,
		"security.rate_limit.window":                        60,
		"security.load_shed.enabled":                        false,
		"security.load_shed.lower_threshold":                0.85,
		"security.load_shed.upper_threshold":                0.95,
		"security.circuit_breaker.enabled":                  false,
		"security.circuit_breaker.failure_threshold":        5,
		"security.circuit_breaker.timeout":                  10,
		"security.circuit_breaker.success_threshold":        5,
		"security.circuit_breaker.half_open_max_concurrent": 1,
		"security.log_viewer.enabled":                       false,
		"security.log_viewer.path":                          "storage/logs",
		"security.log_viewer.username":                      "",
		"security.log_viewer.password":                      "",
		"security.log_viewer.allow_delete":                  false,
		"services.dependencies.database.critical":           true,
		"services.dependencies.cache.critical":              false,
		"services.dependencies.mail.critical":               false,
		"services.dependencies.queue.critical":              false,
		"services.dependencies.search.critical":             false,
		"services.dependencies.storage.critical":            false,
		"services.dependencies.realtime.critical":           false,
		"services.dependencies.backup.critical":             false,
		"i18n.enabled":                                      true,
		"i18n.default_language":                             "en",
		"i18n.supported_languages":                          []string{"en", "zh-CN"},
		"i18n.language_dir":                                 "./lang",
		"i18n.cookie_name":                                  "lang",
		"i18n.cookie_max_age":                               86400,
		"search.enabled":                                    true,
		"search.default":                                    "meilisearch",
		"search.host":                                       "",
		"search.api_key":                                    "",
		"otel.service_name":                                 "lfiber",
		"otel.exporter_type":                                "stdout",
		"otel.endpoint":                                     "localhost:4317",
		"otel.otlp_insecure":                                true,
		"otel.trace_sample_ratio":                           1.0,
		"otel.metrics_enabled":                              true,
		"otel.metrics_path":                                 "/metrics",
	}
}
